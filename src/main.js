  ...
  function updateUI(
    ...
    if (monteCarloEnabled && monteCarloResults) {
      ...
      const mcSummaryLines = [
        ...
        `Trials below target spend: ${((monteCarloResults.pctTrialsBelowTargetSpend ?? 0) * 100).toFixed(1)}%`,
        `Trials hitting min spend: ${((monteCarloResults.pctTrialsHittingMinSpend ?? 0) * 100).toFixed(1)}%`,
        `Median years below target spend: ${(monteCarloResults.medianYearsBelowTargetSpend ?? 0).toFixed(1)}`,
        `Median average annual spend: ${formatCurrency(monteCarloResults.medianAverageAnnualSpend ?? 0)}`,
