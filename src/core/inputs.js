export function readScenarioInputs(doc, getValidatedSpendingSchedule) {
    ...
    const targetSuccessRate = Math.max(0.5, Math.min(0.99, (parseFloat(doc.getElementById("targetSuccess").value) || 90) / 100));
    const solvePrecision = Math.max(10, parseFloat(doc.getElementById("solvePrecision").value) || 100);
    const lifeExpectancy = Math.max(age, Math.min(120, parseInt(doc.getElementById("lifeExpectancy").value) || 100));
    ...
    const selectedCPPMonthly = cppScenarioAge === 60
    ...
    const mcVolatility = Math.max(0, parseFloat(doc.getElementById("mcVolatility").value) / 100 || 0);
    const mcInflationVolatility = Math.max(0, parseFloat(doc.getElementById("mcInflationVolatility").value) / 100 || 0);
