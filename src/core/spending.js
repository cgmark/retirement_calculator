...
export function normalizeScheduleRows(rawRows, currentAge, lifeExpectancy) {
  ...
  const cleaned = (Array.isArray(rawRows) ? rawRows : [])
    .filter(
      (r) =>
        Number.isFinite(r.startAge) &&
        Number.isFinite(r.endAge) &&
    ...
    .map((r) => {
      ...
      const clampedStart = Math.max(
        currentAge,
        Math.min(lifeExpectancy, next.startAge),
      );
      const clampedEnd = Math.max(
        currentAge,
        Math.min(lifeExpectancy, next.endAge),
      );
      if (clampedStart !== next.startAge || clampedEnd !== next.endAge)
        wasClamped = true;
...
export function getScheduleValidationError(cleanedRows) {
  for (let i = 0; i < cleanedRows.length; i++) {
    ...
    if (i > 0 && cleanedRows[i].startAge <= cleanedRows[i - 1].endAge)
      return "overlap";
  }
  return null;
}
...
export function buildFlatSchedule(currentAge, lifeExpectancy, spend) {
  return [
    { startAge: currentAge, endAge: lifeExpectancy, amount: Math.round(spend) },
  ];
}
...
export function isFlatSchedule(rows, currentAge, lifeExpectancy, spend) {
  ...
  const row = rows[0];
  if (
    !Number.isFinite(row.startAge) ||
    !Number.isFinite(row.endAge) ||
    !Number.isFinite(row.amount)
  )
    return false;
  return (
    row.startAge === currentAge &&
    row.endAge === lifeExpectancy &&
    Math.round(row.amount) === Math.round(spend)
  );
}
