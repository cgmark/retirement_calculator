...
export function chooseOutcomeMix(params) {
  ...
  const available = ["tfsa", "nonreg", "rrsp"].filter(
    (acc) =>
      (acc === "tfsa"
        ? currentState.tfsa
        : acc === "nonreg"
  ...
  const horizonScoreForMix = (mix) => {
    ...
    const runYear = (ageAtYear, yearInflationFactor, needNet, forcedMix) => {
      ...
      if (needNet > 0.01)
        ["tfsa", "nonreg", "rrsp"].forEach((acc) => drawSim(acc, needNet));
    ...
    for (let y = yearOffset; startAge + y <= lifeExpectancy; y++) {
      ...
      const targetY =
        getBaseSpendingForAge(ageY, baseSpending, activeSchedule) * inflY;
      ...
      if (y === yearOffset) {
        ...
        if (enforceRrifMin && ageY >= rrifStartAge && sRrsp > 0) {
          ...
          if (rrifMinRate > 0) {
            ...
            const mandatoryTax =
              calculateTax(sTaxable + mandatoryGross, provCode, inflY) -
              calculateTax(sTaxable, provCode, inflY);
  ...
  const evaluateCandidates = (step, centerMix = null, radius = 1) => {
    ...
    for (let pTfsa = tfsaMin; pTfsa <= tfsaMax + 1e-9; pTfsa += step) {
      ...
      const nonMax = centerMix
        ? Math.min(1 - pTfsa, centerMix.nonreg + radius)
        : 1 - pTfsa;
      for (let pNon = nonMin; pNon <= nonMax + 1e-9; pNon += step) {
        const pRrsp = 1 - pTfsa - pNon;
        if (pRrsp < -1e-9) continue;
        const mix = {
          tfsa: Math.max(0, pTfsa),
          nonreg: Math.max(0, pNon),
          rrsp: Math.max(0, pRrsp),
        };
        if (available.length > 0 && available.every((acc) => mix[acc] < 0.001))
          continue;
        candidates.push({ mix });
      }
    }
  ...
  if (dominantSingleObjective) {
    ...
    candidates = [
      ...
      (c) =>
        available.length === 0 || !available.every((acc) => c.mix[acc] < 0.001),
    );
  } else {
  ...
  if (!dominantSingleObjective) {
    const refined = evaluateCandidates(0.1, bestMix, 0.2);
